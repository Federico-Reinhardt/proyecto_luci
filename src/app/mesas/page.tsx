...
import { mesas, getInstitucion } from "@/data/mock";
import { formatFecha } from "@/lib/format";
import { colorEstadoMesa } from "@/lib/badges";

export default function MesasPage() {
  const mesasOrdenadas = [...mesas].sort((a, b) => b.fecha.localeCompare(a.fecha));
          ...
          <tbody className="divide-y divide-slate-100">
            {mesasOrdenadas.map((mesa) => (
              <tr key={mesa.id} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-medium text-slate-900">{getInstitucion(mesa.institucionId)?.nombre}</td>
          ...
          <Link key={mesa.id} href={`/mesas/${mesa.id}`}>
            <Card className="p-4">
              <div className="flex items-center justify-between gap-2">
                <p className="font-medium text-slate-900">{getInstitucion(mesa.institucionId)?.nombre}</p>
                <Badge color={colorEstadoMesa(mesa.estado)}>{mesa.estado}</Badge>
              </div>
